...
from elementum.tmdb_image_http_proxy import TMDB_IMAGE_URL, TMDB_PROXY_URL
...
def dict_replace_value(_dict: dict, old: str, new: str) -> dict:
    x = {}
...
def list_replace_value(_list: list, old: str, new: str) -> list:
    x = []
    for e in _list:
...
def run(url_suffix="", retry=0):
    ...
    proxied_items = list_replace_value(data["items"], TMDB_IMAGE_URL, TMDB_PROXY_URL)
    data["items"] = proxied_items
    listitems = list(range(len(data["items"])))
    log.info("==========DATA ITEMS BEGIN===========")
    log.info(data["items"])
    log.info("==========DATA ITEMS END===========")
